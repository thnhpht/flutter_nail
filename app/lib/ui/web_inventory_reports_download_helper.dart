...
Future<void> downloadInventoryReportsPdfOnWeb(
  ...
  try {
    // Tạo tên file cho báo cáo sản lượng
    final fileName =
        'BaoCaoSanLuong_${DateTime.now().millisecondsSinceEpoch}.pdf';
    ...
    AppWidgets.showFlushbar(
      context,
      AppLocalizations.of(context)!.exportInventoryReportsSuccess,
      type: MessageType.success,
    );
  } catch (e) {
    // Fallback: sử dụng Blob và createObjectUrl
    try {
      await _downloadInventoryReportsPdfOnWebFallback(context, pdfBytes);
    } catch (fallbackError) {
      AppWidgets.showFlushbar(
        context,
        AppLocalizations.of(context)!.exportInventoryReportsError,
...
// Phương thức fallback cho download PDF báo cáo sản lượng trên web
Future<void> _downloadInventoryReportsPdfOnWebFallback(
    BuildContext context, Uint8List pdfBytes) async {
  try {
    // Tạo tên file
    final fileName =
